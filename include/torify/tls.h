...
namespace nodepp { class tls_torify_t {
private:
...
protected:
    ...
    ptr_t<char> htons( uint_16 host_short ) const noexcept {
...
public:
    ...
    event_t<ptr_t<tls_torify_t>,ssocket_t> onSocket ;
    event_t<>                              onClose  ;
   ...
   ~tls_torify_t() noexcept { if( obj.count() > 1 ){ return; } free(); }
    ...
    expected_t<tls_torify_t,except_t>
    connect( string_t host, int port, NODE_CLB clb=nullptr ) const noexcept {
        ...
        if( prxy.null() ){ 
            except_t err = "Error Proxy not found";
            onError.emit(err); return err; 
        }

        if( obj->ctx.create_client()==-1 || host.empty() ){ 
...
namespace nodepp { namespace torify { namespace tls {
